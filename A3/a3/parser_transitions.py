...
class PartialParse(object):
    ...
    def parse_step(self, transition):
        ...
        S -> removes the first word from the buer and pushes it onto the stack.
        LA-> marks the second item on the stack as a dependent of the firrst item and removes the second item from the stack
        RA -> marks the first item on the stack as a dependent of the seconditem and removes the first item from the stack
        """
...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses)>0:
        current_parsers = unfinished_parses[0:batch_size]
        current_transitions = model.predict(current_parsers)
        
        for j in range(0,len(current_parsers)):
            current_parsers[j].parse_step(current_transitions[j])

            if len(current_parsers[j].buffer)==0 and len(current_parsers[j].stack)==1:
                unfinished_parses.remove(current_parsers[j])
